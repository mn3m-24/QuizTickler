...
const ResultPage = () => {
  ...
  return (
    <div className="flex-1 flex flex-col items-center gap-6">
      ...
      <div>
       {((correctCount / questionsLength) * 100).toFixed(2)}% Accuracy
      </div>
      <div className="flex flex-col overflow-y-auto max-h-150 max-w-250 gap-6">
        <div className="flex justify-between">
          <h3>Question Review</h3>
          <h3>{correctCount} Correct / {questionsLength} Total</h3>
        </div>
        <div>
          {questions.map((q, i) => <ReviewQuestion key={i} question={q} userAnswer={answers[i]} />)}
        </div>
      </div>
